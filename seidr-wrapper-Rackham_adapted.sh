...
# project vars
account=naiss2023-22-119
mail=faruk.dube@slu.se
...
EXEC=/home/bastian/seidr-devel/build
...
# 20 workers on 1 nodes (Rackam has 20 cores per node) - setting -n 2 -c 14 (14 cores on 2 nodes, results in the same)
default="-p node -n 20 -t 1-00:00:00"

arguments=(
  [0]=$default
  [1]=$default
  [2]="-p node -n 40 -t 2-00:00:00"
  [3]="-p node -n 20 -t 5-00:00:00"
  [4]=$default
  [5]="-p node -n $correlationNCPUs -t 12:00:00"
  [6]="-p node -n $correlationNCPUs -t 12:00:00"
  [7]=$default
  [8]="-p node -c $correlationNCPUs -t 12:00:00"
  [9]="-p node -n 80 -t 7-00:00:00"
  [10]="-p node -n $correlationNCPUs -t 12:00:00")
